...
class BsplineOptimizer
{
    public:
        ...
        void setGuidePath(const std::vector<Eigen::Vector3d>& guide_pts);
        void setWaypoints(const std::vector<Eigen::Vector3d>& waypts,
                          const std::vector<int>& waypt_idx);

        static double z_constraint(unsigned n, const double* x, double* grad, void* data) {
            double initial_z_value = *(double*)data;
            double z_value = x[2]; 
            if (grad) {
                grad[0] = 0; 
                grad[1] = 0; 
            ...
            return z_value - initial_z_value; 
        }

        void optimize();
        Eigen::MatrixXd getControlPoints();
        std::vector<Eigen::Vector3d> matrixToVector(const Eigen::MatrixXd& ctrl_pts);
    ...
    private:
        ...
        Eigen::Vector3d end_pt;              //终点
        int dim_;                            //维度
        std::vector<Eigen::Vector3d>    guide_pts_;  //几何引导路径点 N-6
        std::vector<Eigen::Vector3d>    waypoints_;  //路径点 N-6
        ...
        std::vector<Eigen::Vector3d> g_q_;//引导路径点
        std::vector<Eigen::Vector3d> g_smoothness_;//平滑度
        std::vector<Eigen::Vector3d> g_distance_;//距离
        std::vector<Eigen::Vector3d> g_feasibility_;//可行性
        std::vector<Eigen::Vector3d> g_endpoint_;//终点
        std::vector<Eigen::Vector3d> g_guide_;//引导
        std::vector<Eigen::Vector3d> g_waypoints_;//航点
        ...
        void calcEndpointCost(const std::vector<Eigen::Vector3d>& q, double& cost, std::vector<Eigen::Vector3d>& gradient);
        void calcGuideCost(const std::vector<Eigen::Vector3d>& q, double& cost, std::vector<Eigen::Vector3d>& gradient);
        void calcVisibilityCost(const std::vector<Eigen::Vector3d>& q, double& cost, std::vector<Eigen::Vector3d>& gradient);
        void calcWaypointsCost(const std::vector<Eigen::Vector3d>& q, double& cost, std::vector<Eigen::Vector3d>& gradient);
