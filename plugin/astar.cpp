...
void Astar::setParam(std::string package_path)
{
    YAML::Node config = YAML::LoadFile(package_path + "/config/astar.yaml");
    resolution_ = config["resolution_astar"].as<double>();
    time_resolution_ = config["time_resolution"].as<double>();
    lambda_heu_ = config["lambda_heu"].as<double>();
    margin_ = config["margin"].as<double>();
    allocate_num_ = config["allocate_num"].as<int>();

    tie_breaker_ = config["tie_breaker"].as<double>();
}
