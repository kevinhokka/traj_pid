...
void TopologyPRM::init(std::string package_path)
{
    YAML::Node config = YAML::LoadFile(package_path + "/config/topo_prm.yaml");
    graph_.clear();
    eng_ = default_random_engine(rd_());
    rand_pos_ = uniform_real_distribution<double>(-1.0, 1.0);

    sample_inflate_(0) = config["sample_inflate"]["x"].as<double>();
    sample_inflate_(1) = config["sample_inflate"]["y"].as<double>();
    sample_inflate_(2) = config["sample_inflate"]["z"].as<double>();
    clearance_ = config["clearance"].as<double>();
    short_cut_num_ = config["short_cut_num"].as<int>();
    reserve_num_ = config["reserve_num"].as<int>();
    ratio_to_short_ = config["ratio_to_short"].as<double>();
    max_sample_time_ = config["max_sample_time"].as<double>();
    max_sample_num_ = config["max_sample_num"].as<int>();
    max_raw_path_ = config["max_raw_path"].as<int>();
    max_raw_path2_ =  config["max_raw_path2"].as<int>();
    parallel_shortcut_ = config["parallel_shortcut"].as<bool>();
...
void TopologyPRM::findTopoPath(Eigen::Vector3d start, Eigen::Vector3d end, std::vector<Eigen::Vector3d> start_pts,
                          ...
                          std::vector<std::vector<Eigen::Vector3d> >& selected_paths)
{
    ...
    graph_time = std::chrono::duration<double>(std::chrono::steady_clock::now() - t1).count();
    ...
    search_time = std::chrono::duration<double>(std::chrono::steady_clock::now() - t1).count();
    ...
    short_time = std::chrono::duration<double>(std::chrono::steady_clock::now() - t1).count();
    ...
    prune_time = std::chrono::duration<double>(std::chrono::steady_clock::now() - t1).count();

    /* ---------- select N shortest paths ---------- */
    t1 = std::chrono::steady_clock::now();

    selected_paths = selectShortPaths(filtered_paths, 1);

    select_time = std::chrono::duration<double>(std::chrono::steady_clock::now() - t1).count();

    final_paths_ = selected_paths;
...
void TopologyPRM::printGraphNodeList(const std::list<GraphNode::Ptr>& graph)
{
...
void TopologyPRM::printGraphNode(const GraphNode::Ptr& g)
{
    std::cout<<"[Topo] id: "<<g->id_<<", type: "<<g->type_<<", pos: "<<g->pos_.transpose()<<std::endl;
    switch(g->state_)
    {
        case GraphNode::NEW:
        ...
        case GraphNode::OPEN:
            ...
            break;
        case GraphNode::CLOSE:
            std::cout<<"state: CLOSE"<<std::endl;
            break;
    }
    std::cout<<"neighbors: ";
    for(const auto& neighbor : g->neighbors_)
    {
...
std::list<GraphNode::Ptr> TopologyPRM::createGraph(Eigen::Vector3d start, Eigen::Vector3d end)
{
    graph_.clear();
    // collis_.clear();
    ...
    while (sample_time < max_sample_time_ && sample_num < max_sample_num_) {
        ...
        if (dist <= clearance_) {
            sample_time += std::chrono::duration<double>(std::chrono::steady_clock::now() - t1).count();
            continue;
        }
...
std::vector<GraphNode::Ptr> TopologyPRM::findVisibGuard(Eigen::Vector3d pt)
{
    std::vector<GraphNode::Ptr> visib_guards;
    Eigen::Vector3d pc;

    int visib_num = 0;
    for(std::list<GraphNode::Ptr>::iterator iter = graph_.begin(); iter != graph_.end(); ++iter)
    {
